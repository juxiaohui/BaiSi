...
static XHNetworkStatus     networkStatus;

static NSTimeInterval      requestTimeout = XH_REQUEST_TIMEOUT;
...
+ (void)updateRequestSerializerType:(XHSerializerType)requestType responseSerializer:(XHSerializerType)responseType {
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    
    if (requestType) {
        switch (requestType) {
            case XHHTTPSerializer: {
                manager.requestSerializer = [AFHTTPRequestSerializer serializer];
                break;
            }
            case XHJSONSerializer: {
...
+ (XHURLSessionTask *)requestWithUrl:(NSString *)url
    ...
    XHURLSessionTask *session;
    
    if (httpMethod == XHPOSTRequest) {
        
        id response = [XHNetWorkRequest getCacheResponseWithURL:url
                                                          params:params];
        
        successBlock && response && useCache ? successBlock(response) : nil;
        
        if (networkStatus == XHNetworkStatusNotReachable ||  networkStatus == XHNetworkStatusUnknown) {
            failBlock ? failBlock(XH_ERROR) : nil;
    ...
    }else if(httpMethod == XHGETRequest){
...
+ (XHURLSessionTask *)uploadWithImage:(UIImage *)image
    ...
    XHURLSessionTask *session = [manager POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (void)detectNetworkStaus {
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable){
            networkStatus = XHNetworkStatusNotReachable;
        }else if (status == AFNetworkReachabilityStatusUnknown){
            networkStatus = XHNetworkStatusUnknown;
        }else if (status == AFNetworkReachabilityStatusReachableViaWWAN || status == AFNetworkReachabilityStatusReachableViaWiFi){
            networkStatus = XHNetworkStatusNormal;
        }
    }];
}
